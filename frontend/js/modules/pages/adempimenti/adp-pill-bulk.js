...
function eliminaBulkPill() {
  ...
  if (!ids.length) {
    showNotif("Seleziona almeno un periodo", "error");
    return;
  }
...
function _aggiornaPillBulkUI() {
  document.querySelectorAll(".periodo-pill[data-id]").forEach(function (p) {
    ...
    if (_pillBulkAttivo) {
      p.style.outline = _pillBulkSelezione.has(id)
        ? "2.5px solid var(--red,#dc2626)"
        : "";
      p.style.background = _pillBulkSelezione.has(id)
        ? "rgba(220,38,38,.07)"
        : "";
      if (!cb) {
        ...
        cb.style.cssText =
          "position:absolute;top:5px;left:5px;width:14px;height:14px;cursor:pointer;accent-color:var(--red,#dc2626);z-index:2";
  ...
  socket.on("res:elimina:adempimenti_cliente_bulk", function (data) {
    if (_pillBulkAttivo) {
      if (data.success) {
        ...
        disattivaModalitaSelezione();
        if (
          _pillBulkContesto === "scad" &&
          typeof loadScadenzario === "function"
        ) {
          loadScadenzario();
        } else if (
          _pillBulkContesto === "globale" &&
          typeof applyGlobaleFiltri === "function"
        ) {
          applyGlobaleFiltri();
        }
      } else {
