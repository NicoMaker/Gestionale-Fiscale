...
function renderImportoCellCompact(r) {
  ...
  if (isContabilita(r)) {
    ...
    const iva = ivaNum !== null ? `<span style="color:${ivaNum < 0 ? "var(--red)" : "var(--green)"}">${formattaNumeroItaliano(ivaNum)}&euro;</span>` : null;
    const acc2 = acc2Num !== null ? `<span style="color:${acc2Num < 0 ? "var(--red)" : "var(--green)"}">${formattaNumeroItaliano(acc2Num)}&euro;</span>` : null;
  ...
  if (hasRate(r)) {
    ...
    const s = sNum !== null ? `<span style="color:${sNum < 0 ? "var(--red)" : "var(--green)"}">${formattaNumeroItaliano(sNum)}&euro;</span>` : null;
    const a1 = a1Num !== null ? `<span style="color:${a1Num < 0 ? "var(--red)" : "var(--green)"}">${formattaNumeroItaliano(a1Num)}&euro;</span>` : null;
    const a2 = a2Num !== null ? `<span style="color:${a2Num < 0 ? "var(--red)" : "var(--green)"}">${formattaNumeroItaliano(a2Num)}&euro;</span>` : null;
...
function renderCheckboxPill(r) {
  ...
  const nomeHtml = r.adempimento_nome ? `<div class="pp-nome" style="color:${color};font-size:11px;line-height:1.3;margin-bottom:2px">${r.adempimento_nome}</div>` : "";
...
function renderPeriodoPill(r) {
  ...
  if (r.data_scadenza || r.data_completamento) {
    ...
    if (r.data_scadenza) dateLine += `<span class="pp-date" title="Data scadenza">📅 ${formattaDataItaliana(r.data_scadenza)}</span>`;
    if (r.data_completamento) dateLine += `<span class="pp-date" style="color:var(--green)" title="Data completamento">✅ ${formattaDataItaliana(r.data_completamento)}</span>`;
    dateLine += `</div>`;
  }
...
function _buildContabilitaLabel(r, pillColor) {
  ...
  const ivaVal = hasIva ? `<span style="color:${ivaNum < 0 ? "var(--red)" : "var(--green)"}">${formattaNumeroItaliano(ivaNum)}&euro;</span>` : "&mdash;";
...
function _buildRateLabel(r, pillColor) {
  ...
  const rows = [
    ...
    const display = done ? `<span style="color:${item.num < 0 ? "var(--red)" : "var(--green)"}">${formattaNumeroItaliano(item.num)}&euro;</span>` : "&mdash;";
...
function renderClienteInfoBox(cliente) {
  ...
  const tipBadge = cliente.tipologia_codice ? `<span class="badge b-${(cliente.tipologia_codice || "").toLowerCase()}" title="${TIPOLOGIE_INFO[cliente.tipologia_codice]?.desc || cliente.tipologia_codice}">${cliente.tipologia_codice}</span>` : "";
  const sottotipoLabel = getLabelSottotipologia(cliente);
  const subBadge = sottotipoLabel ? `<span class="badge b-categoria" title="Sottotipologia: ${sottotipoLabel}">📋 ${sottotipoLabel}</span>` : "";
  let classCols = "";
...
function renderClienteDatiRiferimento(cliente) {
  if (!cliente) return "";
  const items = [];
  const add = (icon, label, val, title) => items.push(`<div class="dati-ref-item" title="${title || label + " " + val}"><span class="ref-icon">${icon}</span><span class="ref-label">${label}</span><span class="ref-value">${val}</span></div>`);
  if (cliente.codice_fiscale) add("🆔", "CF:", cliente.codice_fiscale, "Codice Fiscale");
  if (cliente.partita_iva) add("🏢", "P.IVA:", cliente.partita_iva.replace(/[^0-9]/g, ""), "Partita IVA");
  if (cliente.email) add("📧", "Email:", cliente.email, "Indirizzo email");
  if (cliente.telefono) add("📞", "Tel:", cliente.telefono.replace(/[^0-9]/g, ""), "Numero di telefono");
  ...
  if (cliente.cap) add("📮", "CAP:", cliente.cap.replace(/[^0-9]/g, ""), "Codice di Avviamento Postale");
  if (cliente.indirizzo) add("📍", "Indirizzo:", `${cliente.indirizzo}${cliente.citta ? `, ${cliente.citta}` : ""}${cliente.cap ? ` (${cliente.cap.replace(/[^0-9]/g, "")})` : ""}`, "Indirizzo completo");
  if (cliente.note) add("📝", "Note:", cliente.note.length > 50 ? cliente.note.substring(0, 50) + "..." : cliente.note, cliente.note);
