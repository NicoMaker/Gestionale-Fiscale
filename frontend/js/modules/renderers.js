...
function renderClienteInfoBox(cliente) {
  ...
  if (cliente.col2_value || cliente.col3_value || cliente.periodicita) {
    ...
    if (cliente.periodicita) classCols += `<span class="class-pill per-pill" title="Periodicità contabile"><span class="cp-num">4</span>${cliente.periodicita === "mensile" ? "📅 Mensile" : "📆 Trimestrale"}</span>`;
    classCols += `</div>`;
  }
...
function renderClienteDatiRiferimento(cliente) {
  if (!cliente) return "";
  const items = [];
  const add = (icon, label, val, title) => items.push(
