...
function getSottotipologiePerTipologia(id_tipologia) {
  return queryAll(`SELECT * FROM sottotipologie WHERE id_tipologia = ?`, [id_tipologia]);
}
...
function creaAdempimentiEsempio() {
  ...
  const adempimenti = [
    { codice: 'DICHI_IVA', nome: 'Dichiarazione IVA', scadenza_tipo: 'annuale', is_contabilita: 1 },
    { codice: 'LIQU_IVA', nome: 'Liquidazione IVA', scadenza_tipo: 'trimestrale', is_contabilita: 1 },
    { codice: 'UNICO', nome: 'Modello Unico', scadenza_tipo: 'annuale', is_contabilita: 1 },
    { codice: '770', nome: 'Modello 770', scadenza_tipo: 'annuale', is_contabilita: 1 },
    { codice: 'F24', nome: 'F24', scadenza_tipo: 'mensile', is_contabilita: 0 },
    { codice: 'RAPPRESENTANTE', nome: 'Nomina Rappresentante', scadenza_tipo: 'annuale', is_checkbox: 1 },
    { codice: 'VERBALE_ASS', nome: 'Verbale Assemblea', scadenza_tipo: 'annuale', is_text_only: 1 },
    { codice: 'BILANCIO', nome: 'Bilancio', scadenza_tipo: 'annuale', is_contabilita: 1 },
    { codice: 'IVA_PERIODICA', nome: 'Iva periodica', scadenza_tipo: 'trimestrale', is_contabilita: 1 },
    { codice: 'STUDIO_SETT', nome: 'Studio di settore', scadenza_tipo: 'annuale' },
    { codice: 'SPESOMETRO', nome: 'Spesometro', scadenza_tipo: 'semestrale', is_contabilita: 1 },
  ];

  adempimenti.forEach(a => {
    try {
      adempimentiModel.createAdempimento(a);
    } catch (_) { /* ignora duplicati */ }
  });
...
function generaCliente(tipologie, sottotipologieMap, annoCorrente) {
  ...
  const periodicitaOptions = ['annuale', 'semestrale', 'trimestrale', 'mensile'];
  ...
  return {
    ...
    pec: faker.internet.email({ firstName: 'pec', lastName: cognome }).toLowerCase(),
...
function assegnaAdempimentiACliente(id_cliente, anno, adempimenti) {
  ...
  const numAdp = Math.min(adempimenti.length, Math.floor(Math.random() * adempimenti.length) + 3);
  ...
  scelti.forEach(adp => {
    ...
    righe.forEach(r => {
      ...
      scadenzarioModel.updateAdempimentoStato({
        id: r.id,
        stato,
        data_scadenza: faker.date.future({ years: 1 }).toISOString().split('T')[0],
        data_completamento: stato === 'completato' ? new Date().toISOString().split('T')[0] : null,
...
async function generateTestData() {
  ...
  const scadenze = queryAll(`SELECT id FROM adempimenti_cliente LIMIT 10 OFFSET 5`);
  ...
  const stats = {
    ...
    clientiCestino: queryOne(`SELECT COUNT(*) as cnt FROM cestino WHERE tabella = 'clienti'`),
    adempimentiAttivi: queryOne(`SELECT COUNT(*) as cnt FROM adempimenti WHERE attivo = 1`),
    adempimentiCestino: queryOne(`SELECT COUNT(*) as cnt FROM cestino WHERE tabella = 'adempimenti'`),
    appunti: queryOne(`SELECT COUNT(*) as cnt FROM appunti`),
    appuntiCestino: queryOne(`SELECT COUNT(*) as cnt FROM cestino WHERE tabella = 'appunti'`),
    paginaBianca: queryOne(`SELECT COUNT(*) as cnt FROM pagina_bianca`),
    paginaBiancaCestino: queryOne(`SELECT COUNT(*) as cnt FROM cestino WHERE tabella = 'pagina_bianca'`),
    scadenze: queryOne(`SELECT COUNT(*) as cnt FROM adempimenti_cliente`),
    scadenzeCestino: queryOne(`SELECT COUNT(*) as cnt FROM cestino WHERE tabella = 'adempimenti_cliente'`),
  };
